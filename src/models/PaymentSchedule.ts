...
export class PaymentSchedule {
  ...
  recalculate() {
    ...
    for (let month = 1; month <= termMonths; month++) {
      ...
      // Остаток до вычета дополнительного платежа
      const balanceBeforeExtra = balance.minus(principal).round(2);

      // Остаток после вычета дополнительного платежа
      ...
      const payment = new Payment(
        ...
        interest.toNumber(),
        principal.toNumber(),
        balanceBeforeExtra.toNumber(), // Передаем остаток до доп. платежа
        extra.toNumber(),
        recalcType
      );
      ...
      if (extra.gt(0) && recalcType && balance.gt(0)) {
        if (this.paymentType === 'annuity') {
          ...
          if (recalcType === 'reduceTerm') {
            // Пересчитываем количество оставшихся месяцев, сохраняя monthlyPrincipal неизменным
            const newTerm = balance.div(monthlyPrincipal!).round(0, 2).toNumber();
            remainingMonths = month + newTerm;
            lastPaymentMonth = remainingMonths;
          } else if (recalcType === 'reducePayment') {
            // Пересчитываем monthlyPrincipal для оставшихся месяцев, сохраняя текущий срок
            const remainingMonthsAfter = remainingMonths - month;
            if (remainingMonthsAfter > 0) {
      ...
      // Убедимся, что не превышаем текущий remainingMonths
...
//       if (extra.gt(0) && recalcType === 'reduceTerm' && balance.gt(0)) {
//         let newTerm = 0;
//         let tempBalance = balance;
//         if (this.paymentType === 'annuity') {
//           while (tempBalance.gt(0)) {
...
//           }
//         } else {
//           monthlyPrincipal = balance.div(remainingMonths - month).round(2);
//           newTerm = remainingMonths - month;
//         }
//         remainingMonths = month + newTerm;
//         lastPaymentMonth = remainingMonths;
//       }

//       if (extra.gt(0) && recalcType === 'reducePayment' && balance.gt(0)) {
//         const remainingMonthsAfter = remainingMonths - month;
//         if (remainingMonthsAfter > 0) {
//           if (this.paymentType === 'annuity') {
//             const newLoan = new Loan(balance.toNumber(), this.loan.annualRate.toNumber(), remainingMonthsAfter);
